...
public class S3Uploader {
    ...
    public String S3upload(MultipartFile multipartFile, String filePath) {
        // MultipartFile -> File 로 변환
        File uploadFile = null;
        try {
            uploadFile = convert(multipartFile)
                    .orElseThrow(() -> new ResponseStatusException(HttpStatus.EXPECTATION_FAILED, "파일 변환 실패"));
        } catch (IOException e) {
        ...
        // s3로 업로드 후 로컬 파일 삭제
        String uploadImageUrl = pushToS3(uploadFile, fileName);
        deleteInLocal(uploadFile);
     ...
     * @return 업로드 경로
     */
    public String pushToS3(File uploadFile, String fileName) {
...
*/
     ...
     */
    private void deleteInLocal(File targetFile) {
        if (targetFile.delete()) {
            log.info("Success : 파일 삭제 성공");
            return;
        }
        log.info("Fail : 파일 삭제 실패");
    }
